...
public class Main {
    public static void main(String[] args) {
        ...
        while (esegui) {
            mostraMenu();
            try {
                int sceltaInput = scanner.nextInt();
    ...
    private static void inserisciNuovaPubblicazione(Scanner scanner, Edicola edicola) {
        ...
        while (true) {
            ...
            if (nome.length() >= Pubblicazione.LUNGHEZZA_NOME_MIN)
                break;
            System.out.println("[ERRORE] Il nome deve contenere almeno " + Pubblicazione.LUNGHEZZA_NOME_MIN + " caratteri.");
        }
        ...
        while (true) {
            try {
                ...
                System.out.println("[ERRORE] Le copie ricevute non possono essere negative o uguali a zero.");
            } catch (InputMismatchException e) {
        ...
        while (true) {
            try {
                System.out.print("Prezzo di copertina (€): (maggiore o uguale a " + Pubblicazione.PREZZO_MIN + "): ");
                prezzoCopertina = scanner.nextDouble();
                if(scanner.hasNextLine()) scanner.nextLine();
                if (prezzoCopertina >= Pubblicazione.PREZZO_MIN)
                    break;
                System.out.println("[ERRORE] Il prezzo non può essere inferiore a " + Pubblicazione.PREZZO_MIN + ".");
            } catch (InputMismatchException e) {
        ...
        while (true) {
            try {
                System.out.print("Aggio (percentuale di guadagno " + Pubblicazione.AGGIO_MIN + "-" + Pubblicazione.AGGIO_MAX + "): ");
                aggio = scanner.nextDouble();
                if(scanner.hasNextLine()) scanner.nextLine();
                if (aggio >= Pubblicazione.AGGIO_MIN && aggio <= Pubblicazione.AGGIO_MAX)
                    break;
                System.out.println("[ERRORE] L'aggio deve essere compreso tra " + Pubblicazione.AGGIO_MIN + " e " + Pubblicazione.AGGIO_MAX + ".");
            } catch (InputMismatchException e) {
                System.out.println("[ERRORE] Inserisci un numero decimale valido.");
                if(scanner.hasNextLine()) scanner.nextLine();
            }
        ...
        try {
            switch (sceltaModifica) {
                ...
                case 2 -> {
                    ...
                    int nuoveRicevute = scanner.nextInt();
                    if(scanner.hasNextLine()) scanner.nextLine();
                ...
                case 4 -> {
                    ...
                    double nuovoAggio = scanner.nextDouble();
                    if(scanner.hasNextLine()) scanner.nextLine();
                ...
                case 5 -> {
                    ...
                    int nuoveVendute = scanner.nextInt();
                    if(scanner.hasNextLine()) scanner.nextLine();
